//
//  RSStockProductViewController.m
//  Srms
//
...
@interface RSStockProductViewController (){
...
@implementation RSStockProductViewController

-(void)viewWillAppear:(BOOL)animated{
    [self httpRequestGoodscategory];
    [self addNotificationObserver];
    [self setNaviationModel];
}

- (void) setNaviationModel{
    self.tabBarController.tabBar.hidden = YES;
    [self.currentNavigationController setDIYNavigationBarHidden:NO];
    self.currentNavigationController.titleText = @"库存商品";
...
-(void)categoryLayout{
    ...
    if (_dataDictionary.count > 0) {
        ...
        if (_catoryString.length <= 0 || _catoryString == nil ) {
            ...
            if ([_catoryString isEqualToString:@"KC"]) {
                _catoryString = @"KC";
            }else{
...
//    [cell.shopImage sd_setImageWithURL:[dic objectForKey:@"mainPictureUrl"]];
    ...
    }completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL){        for (UIView * view in cell.shopImage.subviews) {
...
- (void)getGoodsDataWithParamsArray:(NSArray *)array{
    ...
    [self loadProgressViewIsShow:YES];// 加载提示框
