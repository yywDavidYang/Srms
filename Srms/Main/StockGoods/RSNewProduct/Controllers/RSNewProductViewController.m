...
@interface RSNewProductViewController (){
...
@implementation RSNewProductViewController
...
- (void) setNavigationModel{
    ...
    self.currentNavigationController.titleText = @"新品推荐";
...
-(void)categoryLayout{
    ...
    if (_dataDictionary.count > 0) {
        ...
        if (_catoryString.length <= 0 || _catoryString == nil ) {
            ...
            if ([_catoryString isEqualToString:@"XP"]) {
                _catoryString = @"XP";
            }else{
