...
@interface RSHotProductViewController (){
...
@implementation RSHotProductViewController
...
- (void) setNavigationModel{
    ...
    [self.currentNavigationController setDIYNavigationBarHidden:NO];
     ...
     self.currentNavigationController.titleText = @"畅销商品";
...
- (void)viewDidLoad {
    ...
    shopCellView=[[UICollectionView alloc] initWithFrame:CGRectMake(5, 125, self.view.frame.size.width-5, self.view.frame.size.height-164)collectionViewLayout:layout];
    ...
    catoaryTableView.backgroundColor = [UIColor whiteColor];
...
-(void)layOutTopView{
    ...
    buttonDay =[[UIButton alloc] initWithFrame:CGRectMake(140, 10, 40, 30)];
    [buttonDay setBackgroundImage:[UIImage imageNamed:@"dayImage1"] forState:UIControlStateNormal];
    buttonDay.tag = 1;
    [buttonDay addTarget:self action:@selector(selectButton:) forControlEvents:UIControlEventTouchUpInside];
    [topView addSubview:buttonDay];
    
    buttonWeek =[[UIButton alloc] initWithFrame:CGRectMake(190, 10, 40, 30)];
    [buttonWeek setBackgroundImage:[UIImage imageNamed:@"weekImage1"] forState:UIControlStateNormal];
    buttonWeek.tag = 2;
    [buttonWeek addTarget:self action:@selector(selectButton:) forControlEvents:UIControlEventTouchUpInside];
    [topView addSubview:buttonWeek];
    
    buttonMonth =[[UIButton alloc] initWithFrame:CGRectMake(240, 10, 40, 30)];
    [buttonMonth setBackgroundImage:[UIImage imageNamed:@"monthImage2"] forState:UIControlStateNormal];
    buttonMonth.tag = 3;
...
-(void)categoryLayout{
    ...
    if (_dataDictionary.count > 0) {
        ...
        if (_catoryString.length <= 0 || _catoryString == nil ) {
            ...
            if ([_catoryString isEqualToString:@"CX"]) {
                _catoryString = @"CX";
            }else{
...
-(void)selectButton:(UIButton *)sender{
    ...
    switch (sender.tag) {
        case 1:{
            ...
            [buttonDay setBackgroundImage:[UIImage imageNamed:@"dayImage2-1"] forState:UIControlStateNormal];
            [buttonWeek setBackgroundImage:[UIImage imageNamed:@"weekImage1"] forState:UIControlStateNormal];
            [buttonMonth setBackgroundImage:[UIImage imageNamed:@"monthImage1"] forState:UIControlStateNormal];
            [self httpRequestgetGoodsbyfliter];
            break;}
        case 2:{
            sortKeyString = [NSString stringWithFormat:@"%ld",(long)sender.tag];
            [buttonWeek setBackgroundImage:[UIImage imageNamed:@"weekImage2"] forState:UIControlStateNormal];
            [buttonDay setBackgroundImage:[UIImage imageNamed:@"dayImage1"] forState:UIControlStateNormal];
            [buttonMonth setBackgroundImage:[UIImage imageNamed:@"monthImage1"] forState:UIControlStateNormal];
            [self httpRequestgetGoodsbyfliter];
            break;}
...
-(void)httpRequestGoodscategory{
    ...
    [[RSNetworkManager sharedManager] POST:urlString parameters:dic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
